...
export default {
  setup() {
    const spots = ref([]); // Stores the fetched spots data
    ...
    onMounted(async () => {
      ...
      if (!token) {
        error.value = "Please log in to view the featured spots.";
        loading.value = false;
        return;
      }

      try {
        const response = await fetch("http://localhost:3000/api/poppys-pick", {
          method: "GET",
          headers: {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch Poppy's pick spots");
        }
        ...
        error.value = "Failed to load featured spots. Please try again later.";
      } finally {
        loading.value = false;
      }
    });
