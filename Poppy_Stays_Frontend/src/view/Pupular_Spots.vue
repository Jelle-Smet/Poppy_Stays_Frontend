    ...
    onMounted(async () => {
      try {
        const response = await fetch(
          'http://localhost:3000/api/top-booked-spots'
        );
        if (!response.ok) {
          throw new Error('Failed to fetch top booked spots');
        }
        ...
        error.value =
          'Failed to load top booked spots. Please try again later.';
      } finally {
        loading.value = false;
      }
    });
